...
const renderCard = (cardEl) => {
  ...
  const pokeInfo = {
    image: cardEl.querySelector(".pokemon-card-image").src,
    name: cardEl.querySelector(".pokemon-card-title").innerText,
    types: cardEl.querySelector(".pokemon-card-subtitle").innerText,
  }
  
  const infoCard = Mustache.render(infoTemplate, pokeInfo);
  infoContainer.innerHTML = infoCard;
}
    ...
    data.results.forEach((poke) => {
      ...
      fetch(poke.url)
        ...
        .then((data) => {
          ...
          // if card clicked, render it in the info section
          cardEl.addEventListener("click", (event) => {
            renderCard(event.currentTarget);
          })
        })
    });
  })
